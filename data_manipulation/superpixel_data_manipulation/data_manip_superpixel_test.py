...
directory='/home/pbu/Downloads/fire-dataset-dunnings/superpixels/isolated-superpixels/test/'
...
dataset=list()
for items in random.sample(os.listdir(fire_directory),1500):

	items_list=list()
	items_list.append(items)
	items_list.append(1)
	dataset.append(items_list)
for items in random.sample(os.listdir(no_fire_directory),1500):
	items_list=list()
	items_list.append(items)
	items_list.append(0)
	dataset.append(items_list)
...
# Create hdf5 dataset
h5f = h5py.File("{}data_superpixel_test_centre_112.h5".format(output_directory), 'w')
