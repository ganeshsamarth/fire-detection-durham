'''
Inception ResNet v1 keras implementation
'''
...
def inception_resnet_A(input, scale_residual=True):
    ...
    ir_conv = Convolution2D(256, 1, 1, activation='linear', border_mode='same')(ir_concatenate)
...
def inception_resnet_B(input, scale_residual=True):
    ...
    ir_conv = Convolution2D(896, 1, 1, activation='linear', border_mode='same')(ir_concatenate)
    if scale_residual: ir_conv = Lambda(lambda x: x * 0.1)(ir_conv)
...
def inception_resnet_C(input, scale_residual=True):
    ...
    ir_conv = Convolution2D(1792, 1, 1, activation='linear', border_mode='same')(ir_concatenate)
    if scale_residual: ir_conv = Lambda(lambda x: x * 0.1)(ir_conv)
...
def inception_resnet_v1(nb_classes=2, scale=True):
    '''
    Creates a inception resnet v1 network
    :param nb_classes: number of classes.txt
    :param scale: flag to add scaling of activations
    :return: Keras Model with 1 input (299x299x3) input shape and 1 output final_output
    '''
    ...
    model = Model(init, output=out, name='Inception-Resnet-v1')
