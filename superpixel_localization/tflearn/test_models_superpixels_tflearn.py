...
model = model_name(112, 112,normalization,optimizer,dropout,activation,training=False)

print("Constructed ..."+sys.argv[1])
model.load('models_best_superpixels/'+'transfer/'+sys.argv[1]+'/'+sys.argv[1]+'_'+sys.argv[2]+'_'+sys.argv[3]+'_'+str(sys.argv[4])+'_'+sys.argv[5]+'.tflearn')
...
with open('test_results_new.txt','a') as myfile:
	myfile.write('models_new_weights/'+sys.argv[1]+'/'+sys.argv[1]+'_'+sys.argv[2]+'_'+sys.argv[3]+'_'+str(sys.argv[4])+'.tflearn'+'\n')
