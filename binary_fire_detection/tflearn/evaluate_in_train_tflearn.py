...
def evaluate_in_train(model_name,model,num_epoch):
	...
	if num_epoch==0:
		...
		with open('accuracy_thresh.txt','w') as myfile:
			myfile.write(str(acc_thresh)+':'+sys.argv[1]+'_'+sys.argv[2]+'_'+sys.argv[3]+'_'+str(sys.argv[4])+'_'+sys.argv[5]+'.tflearn'+':'+sys.argv[1])
			model.save('models_weights_with_eval/'+sys.argv[1]+'/'+sys.argv[1]+'_'+sys.argv[2]+'_'+sys.argv[3]+'_'+str(sys.argv[4])+'_'+sys.argv[5]+'.tflearn')
